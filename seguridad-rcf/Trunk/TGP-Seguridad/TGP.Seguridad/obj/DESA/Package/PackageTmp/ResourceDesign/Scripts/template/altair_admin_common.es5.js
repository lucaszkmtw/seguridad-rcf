...
function scrollbarWidth() {
    var a = jQuery('<div style="width: 100%; height:200px;">test</div>'),
        ...
        c = a[0],
        a = b[0];jQuery("body").append(a);c = c.offsetWidth;b.css("overflow", "scroll");a = a.clientWidth;b.remove();return c - a;
};
...
function detectIE() {
    var a = window.navigator.userAgent,
        b = a.indexOf("MSIE ");if (0 < b) return parseInt(a.substring(b + 5, a.indexOf(".", b)), 10);if (0 < a.indexOf("Trident/")) return b = a.indexOf("rv:"), parseInt(a.substring(b + 3, a.indexOf(".", b)), 10);b = a.indexOf("Edge/");return 0 < b ? parseInt(a.substring(b + 5, a.indexOf(".", b)), 10) : !1;
};
        ...
        this.setup = (function () {
            ...
            return function () {
                ...
                var $wrapper = this.$wrapper,
                    $input = this.$input;
                if ($input.attr('title')) {
                    $wrapper.attr('title', $input.attr('title')).attr('data-uk-tooltip', $input.attr('data-uk-tooltip'));
                }
            };
        })();
    });
...
// 3.3 page content
altair_page_content = {
    hide_content_sidebar: function hide_content_sidebar() {
    ...
    show_content_sidebar: function show_content_sidebar() {
        if (!$body.hasClass('header_double_height')) {
...
altair_forms = {
    ...
    textarea_autosize: function textarea_autosize() {
    ...
    select_elements: function select_elements(parent) {
        ...
        $selectize_inline.each(function () {
            ...
            if (!$this.hasClass('selectized')) {
                var thisPosBottom = $this.attr('data-md-selectize-bottom');
                $this.after('<div class="selectize_fix"></div>').closest('div').addClass('uk-position-relative').end().selectize({
    ...
    // switchery plugin
    switches: function switches(parent) {
    ...
    parsley_validation_config: function parsley_validation_config() {
    ...
    parsley_extra_validators: function parsley_extra_validators() {
    ...
    dynamic_fields: function dynamic_fields(parent, resize, fields_on_load) {
...
altair_main_sidebar = {
            ...
            if ($body.hasClass('sidebar_mini')) {
                ...
                $document.on('click keyup', function (e) {
                    if ($body.hasClass('sidebar_main_active') && $window.width() < 1220) {
                        if (!$(e.target).closest($sidebar_main).length && !$(e.target).closest($sidebar_main_toggle).length || e.keyCode == 27) {
    ...
    hide_sidebar: function hide_sidebar() {
    ...
    show_sidebar: function show_sidebar() {
    ...
    sidebar_menu: function sidebar_menu() {
        // check for submenu
    ...
    lang_switcher: function lang_switcher() {
        var $lang_switcher = $('#lang_switcher');

        if ($lang_switcher.length) {
            $lang_switcher.selectize({
                options: [{ id: 1, title: 'English', value: 'gb' }, { id: 2, title: 'French', value: 'fr' }, { id: 3, title: 'Chinese', value: 'cn' }, { id: 4, title: 'Dutch', value: 'nl' }, { id: 5, title: 'Italian', value: 'it' }, { id: 6, title: 'Spanish', value: 'es' }, { id: 7, title: 'German', value: 'de' }, { id: 8, title: 'Polish', value: 'pl' }],
                render: {
                    option: function option(data, escape) {
                        return '<div class="option">' + '<i class="item-icon flag-' + escape(data.value).toUpperCase() + '"></i>' + '<span>' + escape(data.title) + '</span>' + '</div>';
                    },
    ...
    swipe_open: function swipe_open() {
    ...
    mini_sidebar: function mini_sidebar() {
    ...
    slim_sidebar: function slim_sidebar() {
        $sidebar_main_toggle.hide();

        $body.addClass('sidebar_slim sidebar_slim_inactive').removeClass('sidebar_main_active sidebar_main_open sidebar_main_swipe');
...
altair_secondary_sidebar = {
    ...
    chat_sidebar: function chat_sidebar() {
...
altair_main_header = {
    ...
    search_activate: function search_activate() {
    ...
    search_show: function search_show() {
    ...
    search_autocomplete: function search_autocomplete() {
...
altair_md = {
    ...
    card_fullscreen: function card_fullscreen() {
    ...
    card_expand: function card_expand() {
        // expand elements
    ...
    card_overlay: function card_overlay() {
        var $md_card = $('.md-card');
        ...
        $md_card.on('click', '.md-card-overlay-toggler', function (e) {
            ...
            if (!$(this).closest('.md-card').hasClass('md-card-overlay-active')) {
                $(this).html('&#xE5CD;').closest('.md-card').addClass('md-card-overlay-active');
            } else {
                $(this).html('&#xE5D4;').closest('.md-card').removeClass('md-card-overlay-active');
            }
        });
    },
    card_single: function card_single() {
        var $md_card_single = $('.md-card-single');
        if ($md_card_single && $body.hasClass('header_double_height')) {
            (function () {
                var md_card_content_height = function md_card_content_height() {
    ...
    card_panel: function card_panel() {
    ...
    card_show_hide: function card_show_hide(card, begin_callback, complete_callback, callback_element) {
        $(card).velocity({
            scale: 0,
            opacity: 0.2
        }, {
    ...
    card_progress: function card_progress(card, percent, steps) {
        ...
        $toolbar_progress.each(function () {
            ...
            $this.css({ 'background': '-moz-linear-gradient(left, ' + bg_color + ' ' + bg_percent + '%, ' + bg_theme + ' ' + bg_percent + '%)' }).css({ 'background': 'linear-gradient(to right,  ' + bg_color + ' ' + bg_percent + '%, ' + bg_theme + ' ' + bg_percent + '%)' }).css({ 'background': '-webkit-linear-gradient(left, ' + bg_color + ' ' + bg_percent + '%, ' + bg_theme + ' ' + bg_percent + '%)' });
        });
    },
    list_outside: function list_outside() {
        var $md_list_outside_wrapper = $('.md-list-outside-wrapper');
        if ($md_list_outside_wrapper && $body.hasClass('header_double_height')) {
            (function () {
                var md_list_outside_height = function md_list_outside_height() {
                    // check header height
    ...
    checkbox_radio: function checkbox_radio(checkbox) {
    ...
    update_input: function update_input(object) {
    ...
    fab_speed_dial: function fab_speed_dial() {
    ...
    fab_toolbar: function fab_toolbar() {
    ...
    wave_effect: function wave_effect() {
...
// common helpers
altair_helpers = {
    truncate_text: function truncate_text($object) {
    ...
    custom_scrollbar: function custom_scrollbar($object) {
    ...
    hierarchical_show: function hierarchical_show(element) {
    ...
    hierarchical_slide: function hierarchical_slide(element) {
    ...
    content_preloader_show: function content_preloader_show(style, variant, container, width, height) {
        ...
        if (!$body.find('.content-preloader').length) {
            ...
            var preloader_content = style == 'regular' ? '<img src="assets/img/spinners/spinner' + image_density + '.gif" alt="" width="32" height="32">' : '<div class="md-preloader"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="' + height + '" width="' + width + '" viewbox="0 0 75 75"><circle cx="37.5" cy="37.5" r="33.5" stroke-width="8"/></svg></div>';
    ...
    content_preloader_hide: function content_preloader_hide() {
        if ($body.find('.content-preloader').length) {
            // hide preloader
    ...
    color_picker: function color_picker(object, pallete, callback) {
        if (object) {
            ...
            for (var $i = 0; $i < cp_pallete_length; $i++) {
                var span = cp_pallete[$i].substring(0, 6) == "md-bg-" ? '<span data-color=' + cp_pallete[$i] + ' class="' + cp_pallete[$i] + '"></span>' : '<span data-color=' + cp_pallete[$i] + ' style="background:' + cp_pallete[$i] + '"></span>';
    ...
    retina_images: function retina_images() {
        if (typeof $.fn.dense !== "undefined") {
            $('img')
            // set resolution cap at "2"
            .attr('data-dense-cap', 2).dense({
                glue: "@",
                ping: false
            });
        }
    ...
    table_check: function table_check() {
    ...
    print_page: function print_page() {
...
// uikit custom
altair_uikit = {
    reinitialize_grid_margin: function reinitialize_grid_margin() {
    ...
    accordion_user_selected: function accordion_user_selected() {
